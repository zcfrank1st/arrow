 ...
 */
fun <S, T, A, B> PPrism<S, T, A, B>.toState() = State<S, Option<A>> { s ->
    s toT getOption(s)
}
 ...
 * Alias for [toState].
 */
fun <S, T, A, B> PPrism<S, T, A, B>.extract(): State<S, Option<A>> = toState()
 ...
 */
inline fun <S, T, A, B, C> PPrism<S, T, A, B>.inspect(crossinline f: (A) -> C): State<S, Option<C>> =
 ...
 * @param f the function you want to apply to the focus.
 */
fun <S, T, A, B> PPrism<S, T, A, B>.modo(f: (A) -> B): IndexedState<S, T, Option<A>> = IndexedState { s ->
 ...
 */
fun <S, T, A, B> PPrism<S, T, A, B>.mod_(f: (A) -> B): IndexedState<S, T, Unit> = IndexedState { s ->
    modify(s, f) toT Unit
}
 ...
 */
fun <S, T, A, B> PPrism<S, T, A, B>.assign(b: B): IndexedState<S, T, Option<B>> = mod { _ -> b }
 ...
 * @param b the value to be set.
 */
fun <S, T, A, B> PPrism<S, T, A, B>.assigno(b: B): IndexedState<S, T, Option<A>> = modo { _ -> b }
 ...
 * @param b the value to be set.
 */
fun <S, T, A, B> PPrism<S, T, A, B>.assign_(b: B): IndexedState<S, T, Unit> = mod_ { _ -> b }