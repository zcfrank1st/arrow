    ...
    init {
        ...
        "Fold select a list that contains one" {
            ...
            forAll(Gen.list(Gen.int())) { ints ->
                (select.getAll(ints).list.firstOrNull() ?: emptyList())
                        .eqv(ints.let { if (it.contains(1)) it else emptyList() })
            }
        }

        "Folding a list of ints" {
            forAll(Gen.list(Gen.int())) { ints ->
                intFold.fold(ints.k()).eqv(ints.sum())
            }
        }

        "Folding a list should yield same result as combineAll" {
            forAll(Gen.list(Gen.int())) { ints ->
                intFold.combineAll(ints.k()).eqv(ints.sum())
            }
        }

        "Folding and mapping a list of strings" {
            forAll(Gen.list(Gen.int())) { ints ->
                stringFold.foldMap(ints.map(Int::toString).k(), String::toInt).eqv(ints.sum())
            }
        }

        "Get all targets" {
            forAll(Gen.list(Gen.int())) { ints ->
                intFold.getAll(ints.k()).eqv(ints.k())
            }
        }

        "Get the size of the fold" {
            forAll(Gen.list(Gen.int())) { ints ->
                intFold.size(ints.k()).eqv(ints.size)
            }
        }

        "Find the first element matching the predicate" {
            forAll(Gen.list(Gen.choose(-100, 100))) { ints ->
                intFold.find(ints.k()) { it > 10 }
        ...
        "Checking existence of a target" {
            forAll(Gen.list(Gen.int()), Gen.bool()) { ints, predicate ->
                intFold.exists(ints.k()) { predicate }.eqv(predicate)
            }
        }

        "Check if all targets match the predicate" {
            forAll(Gen.list(Gen.int())) { ints ->
                intFold.forall(ints.k()) { it % 2 == 0 }
                        .eqv(ints.all { it % 2 == 0 })
            }
        }
        ...
        "Check if there is a target" {
            forAll(Gen.list(Gen.int())) { ints ->
                intFold.nonEmpty(ints.k()).eqv(ints.isNotEmpty())
            }
        }
