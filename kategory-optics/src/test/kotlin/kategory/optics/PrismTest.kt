...
class PrismTest : UnitSpec() {
    ...
    init {
        ...
        "Joining two prisms together with same target should yield same result" {
            forAll(SumGen) { a ->
                val eq = Option.eq(eq<List<Char>>()) //instance lookup eq<Option<List<Char>>>() returns Eq<Option<Char>>

                (sumPrism compose stringPrism).getOption(a).eqv(sumPrism.getOption(a).flatMap(stringPrism::getOption), eq) &&
                        (sumPrism + stringPrism).getOption(a).eqv((sumPrism compose stringPrism).getOption(a), eq)
            }
        }
        ...
        "Checking if a prism exists with a target" {
            forAll(SumGen, SumGen, Gen.bool()) { a, other, bool ->
                ...
                }).isEmpty(other).eqv(bool)
            }
        }

        "Checking if there is no target" {
            forAll(SumGen) { sum ->
                sumPrism.isEmpty(sum).eqv(sum !is SumType.A)
            }
        }

        "Checking if a target exists" {
            forAll(SumGen) { sum ->
                sumPrism.nonEmpty(sum).eqv(sum is SumType.A)
            }
        }

        "Setting a target on a prism should set the correct target"{
            forAll(AGen, Gen.string()) { a, string ->
                sumPrism.setOption(a, string).eqv(a.copy(string = string).some())
            }
        }
        ...
        "Finding a target using a predicate within a Lens should be wrapped in the correct option result" {
            forAll(SumGen, Gen.bool()) { sum, predicate ->
                sumPrism.find(sum) { predicate }.fold({ false }, { true })
                        .eqv(predicate && sum is SumType.A)
            }
        }

        "Checking existence predicate over the target should result in same result as predicate" {
            forAll(SumGen, Gen.bool()) { sum, predicate ->
                sumPrism.exist(sum) { predicate }
                        .eqv(predicate && sum is SumType.A)
            }
        }
        ...
        "Checking satisfaction of predicate over the target should result in opposite result as predicate" {
            forAll(SumGen, Gen.bool()) { sum, predicate ->
                sumPrism.all(sum) { predicate }
                        .eqv(predicate || sum is SumType.B)
            }
        }
