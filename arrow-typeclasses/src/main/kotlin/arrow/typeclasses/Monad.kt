...
interface Monad<F> : Applicative<F>, TC {
     ...
     */
    fun <B> binding(cc: CoroutineContext = EmptyCoroutineContext, c: suspend MonadContinuation<F, *>.() -> B): HK<F, B> {
        val continuation = MonadContinuation<F, B>(this, Platform.awaitableLatch(), cc)
        return flatMap(pure(Unit)) {
