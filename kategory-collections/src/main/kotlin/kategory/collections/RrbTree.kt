...
import kategory.collections.Indented.Companion.arrayString
import kategory.collections.Indented.Companion.indentSpace
...
abstract class RrbTree<E> : BaseList<E>, Indented {
    ...
    class MutRrbt<E> internal constructor(private var focus: Array<E?>,
                                          ...
                                          override var size: Int) : RrbTree<E>(), BaseMutableList<E> {
         ...
         */
        override fun subList(fromIndex: Int, toIndex: Int): MutRrbt<E> =
                super<RrbTree>.subList(fromIndex, toIndex) as MutRrbt<E>
        ...
        /** {@inheritDoc}  */
        override fun without(index: Int): MutRrbt<E> = super.without(index) as MutRrbt<E>
...
//        }
    ...
    class ImRrbt<E> internal constructor(private val focus: Array<E?>,
                                         ...
                                         override val size: Int) : RrbTree<E>(), BaseList<E>, Serializable {
        ...
        private class SerializationProxy<E> internal constructor(@field:Transient() private var rrbTree: RrbTree<E>) : Serializable {
            ...
            private fun readResolve(): Any? = rrbTree
        ...
        /** {@inheritDoc}  */
        override fun concat(es: Iterable<E>): ImRrbt<E> = this.mutable().concat(es).immutable()
        ...
        /** {@inheritDoc}  */
        fun mutable(): MutRrbt<E> =
                MutRrbt(arrayCopy(focus, focus.size, null),
                        focusStartIndex, focus.size,
                        root, size)

        /** {@inheritDoc}  */
        override fun iterator(): Iterator<E> = Iter(pushFocus())
        ...
        /** {@inheritDoc}  */
        override fun without(index: Int): ImRrbt<E> = super.without(index) as ImRrbt<E>
...
*/
    ...
    /** {@inheritDoc}  */
    override abstract fun replace(index: Int, item: E): RrbTree<E>
...
//    abstract override fun size(): Int
     ...
     * Structural changes in the base list make the behavior of the view undefined.
     */
    override fun subList(fromIndex: Int, toIndex: Int): RrbTree<E> = when {
        fromIndex == 0 -> this.split(toIndex).first
        toIndex == size -> this.split(fromIndex).second
        else -> this.split(fromIndex).second.split(toIndex).first
    }
...
//        fun size(): Int = left.size() + leftFocus.size + right.size() + rightFocus.size
    ...
    private class Leaf<T>(internal val items: Array<T?>) : Node<T> {
        ...
        override fun hasStrictCapacity(): Boolean = false

        override fun hasRelaxedCapacity(index: Int, size: Int): Boolean = items.size + size < MAX_NODE_LENGTH
        ...
        override fun numChildren(): Int = size()
        ...
        override fun thisNodeHasRelaxedCapacity(numItems: Int): Boolean = items.size + numItems < MAX_NODE_LENGTH

        override fun toString(): String = arrayString(items)

        override fun indentedStr(indent: Int): String = arrayString(items)
    } // end class Leaf
    ...
    private class Strict<T>(// This is the number of levels below this node (height) times NODE_LENGTH
        ...
        override fun child(childIdx: Int): Node<T> = nodes[childIdx]
        ...
        override fun height(): Int = shift / NODE_LENGTH_POW_2 + 1
         ...
         */
        private fun highBits(i: Int): Int = i shr shift
        ...
        override fun get(i: Int): T? = nodes[highBits(i)][lowBits(i)]

        override fun size(): Int = size

        //        private boolean thisNodeHasCapacity() { return nodes.length < STRICT_NODE_LENGTH; }

        override fun hasStrictCapacity(): Boolean = highBits(size) != STRICT_NODE_LENGTH

        override fun hasRelaxedCapacity(index: Int, size: Int): Boolean = size < MAX_NODE_LENGTH - STRICT_NODE_LENGTH
        ...
        override fun numChildren(): Int = nodes.size
        ...
        override fun thisNodeHasRelaxedCapacity(numItems: Int): Boolean = nodes.size + numItems < MAX_NODE_LENGTH

        override fun toString(): String = "Strict" + shift + arrayString(nodes)
    ...
    internal class Relaxed<T>(
        ...
        override fun height(): Int = nodes[0].height() + 1

        override fun size(): Int = cumulativeSizes[cumulativeSizes.size - 1]
    ...
    private class IdxNode<E>(internal val node: Node<E>) {
        internal var idx = 0
        operator fun hasNext(): Boolean = idx < node.numChildren()

        operator fun next(): Node<E> = node.child(idx++)
...
//                println("Stack: " + Arrays.toString(stack))
    ...
    companion object {
        ...
        private fun iterablesEqual(a: Iterable<*>?, b: Iterable<*>?): Boolean {
        ...
        private fun iterableToString(name: String?, iterable: Iterable<*>?): String {
            if (name == null) {
                throw IllegalArgumentException("Can't have a null name.")
            }
            if (iterable == null) {
                throw IllegalArgumentException("Can't have a null iteratable.")
            }
