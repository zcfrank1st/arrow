import TestUtilities.compareIterators
import TestUtilities.serializeDeserialize
import io.kotlintest.KTestJUnitRunner
import EqualsContract.equalsDistinctHashCode
import kategory.collections.RrbTree.Companion.STRICT_NODE_LENGTH
import kategory.*
...
class RrbTreeTest { // : UnitSpec() {
    ...
    fun insertRandom() {
        ...
        try {
            for (j in 0 until SEVERAL) {
                ...
                control.add(idx, j)
                Assert.assertEquals((j + 1).toLong(), rrb.size.toLong())
                Assert.assertEquals(Integer.valueOf(j), rrb[idx])
            ...
            Assert.assertEquals(SEVERAL.toLong(), rrb.size.toLong())
            for (j in 0 until SEVERAL) {
                Assert.assertEquals(control[j], rrb[j])
            }
        } catch (e: Exception) {
    ...
    private fun randomInsertTest2(r: RrbTree<Int>, control: MutableList<Int>, indices: IntArray): RrbTree<Int> {
        var rrb = r
        Assert.assertEquals("inputSize (if this blows up, this test is being used incorrectly)", control.size.toLong(), rrb.size.toLong())
        ...
        for (j in indices.indices) {
            ...
            Assert.assertEquals("size", control.size.toLong(), rrb.size.toLong())
            Assert.assertEquals("item at " + idx, control[idx], rrb[idx])
            //            System.out.println("control:" + control);
            //            System.out.println("===test:" + is);
            for (k in 0..j) {
                Assert.assertEquals("Wrong item at " + k + ", but still correct size (" + rrb.size + ")\n" +
                                    "control:\n" + control.toString() + "\n" +
                                    "test:\n" + rrb.indentedStr(0),
                                    control[k], rrb[k])
                //                System.out.println("control[" + k + "]:" + control.get(k) + " test[" + k + "]:" + is.get(k));
    ...
    fun insertRandomIntoStrict() {
        ...
        try {
            for (j in 0 until SEVERAL) {
                ...
                rrb = rrb.insert(idx, j)
                control.add(idx, j)
                Assert.assertEquals(control.size.toLong(), rrb.size.toLong())
                Assert.assertEquals(Integer.valueOf(j), rrb[idx])
            }
            Assert.assertEquals(control.size.toLong(), rrb.size.toLong())
    ...
    fun basics() {
        ...
        Assert.assertEquals(0, rrb.size.toLong())

        val rrb1 = rrb.append(5)
        Assert.assertEquals(0, rrb.size.toLong())
        Assert.assertEquals(1, rrb1.size.toLong())
        Assert.assertEquals(Integer.valueOf(5), rrb1[0])

        val rrb2 = rrb1.append(4)
        Assert.assertEquals(0, rrb.size.toLong())
        Assert.assertEquals(1, rrb1.size.toLong())
        Assert.assertEquals(2, rrb2.size.toLong())
        Assert.assertEquals(Integer.valueOf(5), rrb1[0])
        Assert.assertEquals(Integer.valueOf(5), rrb2[0])
        Assert.assertEquals(Integer.valueOf(4), rrb2[1])

        val rrb3 = rrb2.append(3)
        Assert.assertEquals(0, rrb.size.toLong())
        Assert.assertEquals(1, rrb1.size.toLong())
        Assert.assertEquals(2, rrb2.size.toLong())
        Assert.assertEquals(3, rrb3.size.toLong())
        Assert.assertEquals(Integer.valueOf(5), rrb1[0])
        Assert.assertEquals(Integer.valueOf(5), rrb2[0])
        Assert.assertEquals(Integer.valueOf(5), rrb3[0])
        Assert.assertEquals(Integer.valueOf(4), rrb2[1])
        Assert.assertEquals(Integer.valueOf(4), rrb3[1])
        Assert.assertEquals(Integer.valueOf(3), rrb3[2])
        rrb.debugValidate()
        rrb1.debugValidate()
        rrb2.debugValidate()
        rrb3.debugValidate()
    ...
    fun testEmptyIterator() {
        Assert.assertFalse(RrbTree.empty<Any>().iterator().hasNext())
    }
...
//        println("test:" + test.indentedStr(0));
    ...
    fun addSeveralItems() {
        ...
        for (j in 0 until SEVERAL) {
            rrb = rrb.append(j)
            Assert.assertEquals((j + 1).toLong(), rrb.size.toLong())
            Assert.assertEquals(Integer.valueOf(j), rrb[j])
            for (k in 0..j) {
                Assert.assertEquals(Integer.valueOf(k), rrb[k])
            }
            rrb.debugValidate()
        }
        Assert.assertEquals(SEVERAL.toLong(), rrb.size.toLong())
    ...
    fun relaxedSplitTest() {
        ...
        try {
            ...
            Assert.assertEquals(TWO_LEVEL_SZ.toLong(), rrb.size.toLong())
            Assert.assertEquals(TWO_LEVEL_SZ.toLong(), ms.size.toLong())
    ...
    fun replaceTest() {
        ...
        Assert.assertArrayEquals(arrayOf("Hello", "World"),
                                 im.toTypedArray())
        Assert.assertArrayEquals(arrayOf("Hello", "World"),
                                 mu.toTypedArray())

        Assert.assertArrayEquals(arrayOf("Goodbye", "World"),
        ...
        Assert.assertArrayEquals(control, im2.toTypedArray())
        Assert.assertArrayEquals(control, mu2.toTypedArray())
        ...
        Assert.assertArrayEquals(control, im3.toTypedArray())
        Assert.assertArrayEquals(control, mu3.toTypedArray())
    ...
    private fun testReplaceGuts(rrb: RrbTree<Int>) {
        ...
        im.debugValidate()
        for (i in im.indices) {
            Assert.assertEquals(Integer.valueOf(i), im[i])
        }
    }
    ...
    fun listIterator() {
        ...
        for (i in 0 until len) {
            ...
            mu.append(testVal)
            Assert.assertEquals(Integer.valueOf(testVal), im[i])
            Assert.assertEquals(Integer.valueOf(testVal), mu[i])
            test[i] = testVal
        }
        Assert.assertArrayEquals(test, im.toTypedArray())
        Assert.assertArrayEquals(test, mu.toTypedArray())
        Assert.assertArrayEquals(test, serializeDeserialize<Array<Any>>(im.toTypedArray()))
    ...
    fun equalsAndHashCode() {
        ...
        var rrb1 = RrbTree.empty<Int>()
        for (item in control) {
            rrb1 = rrb1.append(item)
        }
        var rrb2 = RrbTree.emptyMutable<Int>()
        for (item in control) {
            rrb2 = rrb2.append(item)
        }
        ...
        var rrb3 = RrbTree.empty<Int?>()
        for (item in hasNull) {
            rrb3 = rrb3.append(item)
        }
        var rrb4 = RrbTree.emptyMutable<Int?>()
        for (item in hasNull) {
            rrb4 = rrb4.append(item)
        }
    ...
    fun coverageJunky() {
        ...
        Assert.assertTrue(s1.contains("RrbTree(size=9 "))
        Assert.assertTrue(s1.contains("        root="))

        Assert.assertEquals("MutRrbt(0,1,2,3,4,5,6,7,8)", rrb1.toString())
        ...
        Assert.assertEquals("MutRrbt(0,7,3,5,1,4,2,8,6)", rrb2.toString())
        ...
        Assert.assertTrue(im.indentedStr(7).startsWith("RrbTree(size=99 fsi=96 focus="))
        Assert.assertTrue(im.indentedStr(7).contains("               root=Strict"))

        Assert.assertTrue(mu.indentedStr(7).startsWith("RrbTree(size=99 fsi=96 focus="))
        Assert.assertTrue(mu.indentedStr(7).contains("               root=Strict"))
    }

    @Test
    fun joinImTest() {
        Assert.assertEquals(rrb(1, 2, 3, 4, 5, 6), rrb(1, 2, 3).join(rrb(4, 5, 6)))
        ...
        Assert.assertEquals(r3, r1.join(r2))
        ...
        for (i in 1 until MAX_ITEMS) {
            ...
            Assert.assertEquals(control, r3)
            r3.debugValidate()
        }
    ...
    fun testBiggerJoin() {
        ...
        Assert.assertEquals(rrb, serializeDeserialize(rrb))
        var splitIndex = 1
        while (splitIndex <= TWO_LEVEL_SZ) {
            val isSplit = rrb.split(splitIndex)
            Assert.assertEquals(rrb, isSplit.first.join(isSplit.second))

            val msSplit = ms.split(splitIndex)
            Assert.assertEquals(ms, msSplit.first.join(msSplit.second))
            splitIndex += (STRICT_NODE_LENGTH.toDouble() * STRICT_NODE_LENGTH.toDouble() * 0.333).toInt()
        }
    }
    ...
    fun testWithout() {
        ...
        Assert.assertEquals(mut(1, 2, 3, 5, 6), mut(1, 2, 3, 4, 5, 6).without(3))
        ...
        for (i in 68 until MAX_ITEMS - 1) { // TODO: Start i = 68 to see issue.
            ...
            Assert.assertEquals(control, im2)
            Assert.assertEquals(control, mu2)
            im2.debugValidate()
            mu2.debugValidate()
            Assert.assertEquals(im2, mu2)
        }
        //        }
    }
    ...
    companion object {
        ...
        private fun buildInOrderTest(r: RrbTree<Int>, iterations: Int): RrbTree<Int> {
            ...
            Assert.assertEquals(control, rrb)
            rrb.debugValidate()
            Assert.assertEquals(iterations.toLong(), rrb.size.toLong())
            for (j in 0 until iterations) {
                Assert.assertEquals(Integer.valueOf(j), rrb[j])
            }
            return rrb
        ...
        private fun buildReverseOrderTest(r: RrbTree<Int>, iterations: Int): RrbTree<Int> {
            ...
            for (j in 0 until iterations) {
                ...
                control.add(0, j)
                Assert.assertEquals((j + 1).toLong(), rrb.size.toLong())
                Assert.assertEquals(Integer.valueOf(j), rrb[0])
                //            System.out.println(" ==" + is);
                for (k in 0..j) {
                    Assert.assertEquals("Checking index: " + k + " for size=" + control.size, control[k], rrb[k])
                }
                rrb.debugValidate()
                //            System.out.println(is);
            }
            Assert.assertEquals(iterations.toLong(), rrb.size.toLong())
            for (j in 0 until iterations) {
                Assert.assertEquals(Integer.valueOf(iterations - j - 1), rrb[j])
            }
